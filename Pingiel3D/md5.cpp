...
void md5load::init(const char *filename, const char **animfile)
{
	...
	{
		string x = string(sciez) + string(md5file.meshes[i].shader) + string(rozsz);
		char* y = new char[x.length() + 1];
		modeltexture[i] = loadTexture(strcpy(y, x.c_str()));
		delete[]y;
	}
	 rozsz = "_local.tga";
	for (int i = 0; i < md5file.num_meshes; i++)
	{
		string x = string(sciez) + string(md5file.meshes[i].shader) + string(rozsz);
		char* y = new char[x.length() + 1];
		modelshader[i] = loadTexture(strcpy(y, x.c_str()));
		delete[]y;
	}
	//modeltexture[1] = loadTexture("Pliki/imp/imp_local.tga");
	AllocVertexArrays();
	...
	for (int i = 0; i < 7; i++)
	{
		if (animfile[i])
		{
			if (!ReadMD5Anim(animfile[i], &md5anim[i]))
			{
				FreeAnim(&md5anim[i]);
				//MessageBox(NULL, "AnimNotloaded", "hehe", MB_OK);
			}
			else
			{
				animInfo[i].curr_frame = 0;
				animInfo[i].next_frame = 1;

				animInfo[i].last_time = 0;
				animInfo[i].max_time = 1.0 / md5anim[i].frameRate;

				/* Allocate memory for animated skeleton */
				skeleton[i] = (struct md5_joint_t *)malloc(sizeof(struct md5_joint_t) * md5anim[i].num_joints);
		...
		else if (sscanf(buff, " numJoints %d", &anim->num_joints) == 1)
		{
			if (anim->num_joints > 0)
			{
				for (i = 0; i < anim->num_frames; ++i)
				{
					...
					anim->skelFrames[i] = (struct md5_joint_t *)malloc(sizeof(struct md5_joint_t) * anim->num_joints);
				}

				/* Allocate temporary memory for building skeleton frames */
				jointInfos = (struct joint_info_t *)malloc(sizeof(struct joint_info_t) * anim->num_joints);

				baseFrame = (struct baseframe_joint_t *)malloc(sizeof(struct baseframe_joint_t) * anim->num_joints);
			}
		}
...
void md5load::draw(float x, float y, float z, float scale, int an)
//void md5load::draw ()
{
	...
	if (animated)
	{
		...
		Animate(&md5anim[an], &animInfo[an], 0.015*speed/3);
		//     /* Interpolate skeletons between two frames */
		//InterpolateSkeletons(md5anim.skelFrames[int(cur_frame)],md5anim.skelFrames[int(nex_frame)],	md5anim.num_joints,	tajm * md5anim.frameRate,skeleton);
		InterpolateSkeletons(md5anim[an].skelFrames[animInfo[an].curr_frame], md5anim[an].skelFrames[animInfo[an].next_frame], md5anim[an].num_joints, animInfo[an].last_time * md5anim[an].frameRate, skeleton[an]);
	}
	else
	{
		/* No animation, use bind-pose skeleton */
		skeleton[an] = md5file.baseSkel;
	}
	...
	{
		DrawSkeleton(skeleton[an], md5file.num_joints);
	}
	...
	for (i = 0; i < md5file.num_meshes; i++)
	{
		...
		glBindTexture(GL_TEXTURE_2D, modelshader[i]);
		glUniform1i(glGetUniformLocation(podstawowy_shader->Out(), "normalmap"), 1);

		PrepareMesh(&md5file.meshes[i], skeleton[an]);

		glVertexPointer(3, GL_FLOAT, sizeof(GL_FLOAT) * 5, vertexArray);

		char *evilPointer = reinterpret_cast<char *>(vertexArray);
...
void md5load::draw(float x, float y, float z, float scale, int an,int speedy)
//void md5load::draw ()
{
	int i;
	if (drawTexture == true)
	...
	{
		//     /* Calculate current and next frames */
		Animate(&md5anim[an], &animInfo[an], 0.015*speedy / 3);
		//     /* Interpolate skeletons between two frames */
		//InterpolateSkeletons(md5anim.skelFrames[int(cur_frame)],md5anim.skelFrames[int(nex_frame)],	md5anim.num_joints,	tajm * md5anim.frameRate,skeleton);
		InterpolateSkeletons(md5anim[an].skelFrames[animInfo[an].curr_frame], md5anim[an].skelFrames[animInfo[an].next_frame], md5anim[an].num_joints, animInfo[an].last_time * md5anim[an].frameRate, skeleton[an]);
	}
...
{
	return skeleton[2][joint].pos[xyz];
}